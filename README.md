...
#Guide to Using the ChatGPT Package from Deep
*1. Opening Gitpod
Navigate to Gitpod link https://github.com/deep-foundation/dev.
...
Insert the following query to log in as admin and press Enter:
```
const adminId = await deep.id('deep', 'admin');
await deep.login({
  linkId: adminId,
});```
Upon successful login, you’ll be presented with a new interface with a links.
...
*4. Granting Admin Rights to the all packages
Insert the following query into the console to grant admin rights to the all packages and press Enter:
```
...
Open its editor and insert your API key, then save the changes (Ctrl+S).
...
If you want to use another model, execute the following query, replacing MODEL with the ID of the model link you want to use:
```
...
You can repeat these steps whenever you want to interact with ChatGPT or create a new dialogue.
